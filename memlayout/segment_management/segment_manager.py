...
from memlayout.segment_management.segment import MemorySegment, CodeSegment, DataSegment
from memlayout.utils.enums import Memory_types, Page_types
...
# from Tool.memory_management.memory_segments import MemorySegment, CodeSegment, DataSegment
...
class SegmentManager:
    def __init__(self, page_table):
        ...
        """
        logger = get_logger()
        logger.info("==================== SegmentManager")


        self.page_table = page_table
        self.memory_segments: List[MemorySegment] = []
        self.pool_type_mapping: Dict[Memory_types, List[MemorySegment]] = {}  # To map pool types to blocks


    def allocate_memory_segment(self, name: str, byte_size:int, memory_type:Memory_types, alignment_bits:int=None, VA_eq_PA:bool=False)->MemorySegment:
        """
        ...
        if memory_type in [Memory_types.DATA_SHARED, Memory_types.DATA_PRESERVE, Memory_types.STACK]:
            page_type = Page_types.TYPE_DATA
        elif memory_type in [Memory_types.CODE, Memory_types.BOOT_CODE, Memory_types.BSP_BOOT_CODE]:
        ...
        if page_type == Page_types.TYPE_CODE:
        ...
        if page_type == Page_types.TYPE_CODE:
            pool = all_intervals.get_intervals(criteria={"page_type": Page_types.TYPE_CODE})
            pool_name = "CODE"
        else:
            pool = all_intervals.get_intervals(criteria={"page_type": Page_types.TYPE_DATA})
            pool_name = "DATA"
        ...
        if (memory_type == Memory_types.CODE) \
            or (memory_type == Memory_types.BOOT_CODE) \
            or (memory_type == Memory_types.BSP_BOOT_CODE):
    ...
    def allocate_cross_core_data_memory_segment():
        ...
        byte_size = 2048 # 2KB
        memory_type = Memory_types.DATA_PRESERVE
        page_type = Page_types.TYPE_DATA
        ...
        all_pages = first_MMU.get_pages_by_type(Page_types.TYPE_DATA)
        cross_core_pages = []
        ...
        all_non_allocated_data_intervals = first_MMU.non_allocated_va_intervals.get_intervals(criteria={"page_type": Page_types.TYPE_DATA})
        contained_intervals = []
        ...
        mmu_manager.allocated_pa_intervals.add_region(shared_pa_start, byte_size, metadata={"page_type": Page_types.TYPE_DATA, "mmu_name": first_MMU.mmu_name})
        ...
        for state_name in all_states:
            for mmu in mmu_manager.get_core_mmus(state_name):
                ...
                all_pages = mmu.get_pages_by_type(Page_types.TYPE_DATA)
                ...
                mmu.allocated_va_intervals.add_region(segment_va_start, byte_size, metadata={"page_type": Page_types.TYPE_DATA, "mmu_name": mmu.mmu_name})
    ...
    def _is_code_page_type(self, page_type):
        ...
        return page_type == Page_types.TYPE_CODE

    def get_segments(self, pool_type:[Memory_types | list[Memory_types]]) -> List[MemorySegment]:
        """
        ...
        for pool_type in pool_types:
            if not isinstance(pool_type, Memory_types):
                logger.warning(f"ID of pool_type's type:", id(type(pool_type)))
                logger.warning(f"ID of Memory_types:", id(Memory_types))
    ...
    def get_stack_data_start_address(self) -> int:
        ...
        """
        stack_blocks = self.get_segments(pool_type=Memory_types.STACK)
